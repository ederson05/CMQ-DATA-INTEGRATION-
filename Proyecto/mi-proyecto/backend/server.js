  ...
  try {
    ...
    const result = await conn.execute(
      `UPDATE TBL_PACIENTE SET
        PAC_NOMBRE = NVL(:nombre, PAC_NOMBRE),
        PAC_TELEFONO = NVL(:telefono, PAC_TELEFONO),
        PAC_EMAIL = NVL(:email, PAC_EMAIL),
        PAC_DIRECCION = NVL(:direccion, PAC_DIRECCION),
        PAC_CIUDAD = NVL(:ciudad, PAC_CIUDAD),
        PAC_EMERGENCIA_NOMBRE = NVL(:contactoEmergenciaNombre, PAC_EMERGENCIA_NOMBRE),
        PAC_EMERGENCIA_TELEFONO = NVL(:contactoEmergenciaTel, PAC_EMERGENCIA_TELEFONO)
       WHERE PAC_DOCUMENTO = :id`,
      {
        nombre,
        telefono,
        email,
    ...
    if (result.rowsAffected === 0) {
      return res.json({ success: false, mensaje: 'No se realizaron cambios' });
    }
